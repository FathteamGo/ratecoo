...
export default function AdminSignInPage() {
  ...
  useEffect(() => {
    ...
    if (getCookie('admin_login_status') === 'authenticated') {
      router.push("/dashboard");
    }
    
    // Clear any previous login status cookie to ensure clean state
    if (getCookie('admin_login_status')) {
      deleteCookie('admin_login_status');
    }
  }, [searchParams, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res?.ok) {
      ...
      setCookie('admin_login_status', 'authenticated', 1); // 1 day expiry
      
      // Delay redirect to ensure session and cookie are properly set
      setTimeout(() => {
